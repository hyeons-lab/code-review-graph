...
`install` detects which AI coding tools you have, writes the correct MCP configuration, and installs platform-native hooks where supported. If Codex is detected, auto-install configures Codex through `~/.codex/config.toml` and skips repo-local MCP configs to avoid duplicate MCP servers in Codex sessions. Restart your editor/tool after installing.

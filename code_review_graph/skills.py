...
def _filter_auto_detected_platforms(
    ...
    # Codex can discover repo-local MCP declarations from several tool config
    # files, not just .mcp.json. Installing those alongside the global Codex
    # config starts duplicate graph MCP servers in worktree sessions, which can
    # stall /review before the MCP tool call is dispatched. Keep explicit
    # platform installs intact; only auto-detect mode skips repo-local configs
    # when Codex is also being configured.
    if "codex" in filtered:
        repo_root = repo_root.resolve()
        for key, plat in list(filtered.items()):
            if key == "codex":
                continue
            try:
                config_path = plat["config_path"](repo_root).resolve()
...
def install_platform_configs(
    ...
    if target == "all":
        ...
        platforms_to_install = _filter_auto_detected_platforms(
            repo_root, platforms_to_install
        )
    else:

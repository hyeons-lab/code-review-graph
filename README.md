...
One command sets up everything. `install` detects which AI coding tools you have, writes the correct MCP configuration, installs platform-native hooks/skills where supported, and injects graph-aware instructions into your platform rules. When Codex is detected, auto-install uses `~/.codex/config.toml` and skips Claude Code's repo-local `.mcp.json` so Codex does not start duplicate graph servers. It auto-detects whether you installed via `uvx` or `pip`/`pipx` and generates the right config. Restart your editor/tool after installing.
